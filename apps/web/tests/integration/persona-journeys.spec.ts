  ...
  test.describe('Customer Journey: Finding and Booking Warehouse Space', () => {
    test('New customer can search, compare, and book warehouse space', async ({ page }) => {
      ...
      await test.step('Homepage exploration', async () => {
        ...
        await page.screenshot({ path: 'screenshots/customer-journey-homepage.png', fullPage: true });
      });

      // 2. Search Phase
      await test.step('Search for warehouses', async () => {
        // Enter search criteria
        await page.fill('[placeholder="Type of space, size, or features..."]', 'climate control storage');
        ...
        await expect(page.locator('[data-testid="warehouse-card"]')).toHaveCount(1, { timeout: 10000 });
      ...
      await test.step('Create account and book warehouse', async () => {
        ...
        await page.fill('[data-testid="storage-requirements"]', 'Need space for e-commerce inventory');
    ...
    test('Existing customer can quickly book additional space', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="warehouse-card"]')).toHaveCount(2, { timeout: 5000 });
      ...
      await expect(page.locator('[data-testid="instant-confirmation"]')).toBeVisible({ timeout: 3000 });
    });
  });
  ...
  test.describe('Cross-Persona Interaction: Dispute Resolution', () => {
    test('Customer and operator can resolve a dispute', async ({ context }) => {
      ...
      await test.step('Customer reports damage', async () => {
        ...
        await customerPage.fill('[data-testid="damage-description"]', 'Damaged inventory during storage');
      ...
      await test.step('Operator responds to dispute', async () => {
        ...
        // Respond
        await operatorPage.fill('[data-testid="operator-response"]', 'We acknowledge the issue and are investigating');
