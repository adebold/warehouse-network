...
export class TestDataFactory {
  ...
  static createOperator(overrides?: Partial<Operator>): Operator {
    return {
      ...
      operatingRegions: faker.helpers.arrayElements(['Ontario', 'Quebec', 'Alberta', 'British Columbia']).join(','),
      warehouseCount: faker.number.int({ min: 1, max: 10 }),
      goodsCategories: faker.helpers.arrayElements(['General', 'Electronics', 'Food', 'Chemical', 'Pharmaceutical']).join(','),
  ...
  static createWarehouse(overrides?: Partial<any>): any {
    ...
    return {
      ...
      city: faker.helpers.arrayElement(['Toronto', 'Mississauga', 'Brampton', 'Hamilton', 'Ottawa']),
      province: 'ON',
      ...
      pricingModel: faker.helpers.arrayElement(['per-sqft-monthly', 'flat-monthly', 'per-pallet-daily']),
  ...
  static createBooking(overrides?: Partial<any>): any {
    ...
    return {
      ...
      goodsCategory: faker.helpers.arrayElement(['General Merchandise', 'Electronics', 'Food Products', 'Raw Materials']),
  ...
  static generateScenarios() {
    return {
      ...
      happyPath: {
        ...
        warehouses: [
          this.createWarehouse({
            ...
            totalSize: 50000,
            availableSize: 25000,
            features: ['24/7 Access', 'Climate Control', 'Security System', 'Loading Docks', 'Office Space'],
            basePrice: 18.50
          }),
          this.createWarehouse({
      ...
      performanceData: {
        ...
        manyWarehouses: Array(1000).fill(null).map(() => this.createWarehouse()),
        
        // Generate concurrent bookings
        concurrentBookings: Array(50).fill(null).map(() => this.createBooking({
          status: 'PENDING',
