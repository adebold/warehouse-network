...
const Inventory: NextPage<InventoryProps> = ({ skids: initialSkids, customer }) => {
  ...
  const filteredSkids = skids.filter(skid => {
    const matchesSearch = skid.skidCode.toLowerCase().includes(searchTerm.toLowerCase()) ||
  ...
  const columns = [
    {
      ...
      accessor: (skid: typeof skids[0]) => (
        <div className="font-medium">{skid.skidCode}</div>
      ),
    },
    {
      key: 'status',
    ...
    },
    {
      ...
      accessor: (skid: typeof skids[0]) => (
        <div className="text-muted-foreground">
          {skid.location?.name || 'No location assigned'}
        </div>
      ),
    },
    {
          ...
          <CardHeader>
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between space-y-4 sm:space-y-0">
              ...
              <div className="flex space-x-2">
                <Button 
                  size="sm" 
                  disabled={customer?.accountStatus === 'LOCKED' || customer?.paymentStatus === 'DELINQUENT'}
